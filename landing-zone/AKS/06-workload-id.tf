...
resource "azurerm_role_assignment" "aks_identity_operator" { # required suivant documentation 
# for_each             = module.aks
...
resource "azurerm_role_assignment" "aks_vm_contributor" { # required suivant documentation
  #for_each             = module.aks
  scope                = "/subscriptions/${data.azurerm_client_config.current.subscription_id}/resourcegroups/${azurerm_kubernetes_cluster.akscluster.node_resource_group}"
...
# on peut utiliser workload id et c'est recommander par microsoft 
resource "azurerm_key_vault_access_policy" "aad_pod_identity" { 
  key_vault_id = data.terraform_remote_state.aks-support.outputs.kv_id
