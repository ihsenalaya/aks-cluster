# # Deploy DNS Private Zone for ACR  and key vault (used for prived endpoint later to avoid disposing resources to internet)
resource "azurerm_private_dns_zone" "acr-dns" {
  name                = "privatelink.azurecr.io"
  resource_group_name = azurerm_resource_group.spoke-rg.name
}

/* Ces liens permettent la résolution et
 l'enregistrement DNS au sein des réseaux virtuels Azure à l'aide d'Azure DNS privé.
 Ce lien permet aux machines virtuelles et autres ressources au sein du réseau virtuel 
 de résoudre les noms de domaine dans cette zone DNS privée.
 */
